...
class MapTrap : public cocos2d::Node // 继承 Node 而不是 Sprite，因为平时它是隐形的
{
public:
    // create 函数：传入触发区域和伤害
    static MapTrap* create(const cocos2d::Rect& area, int damage);
    virtual bool init(const cocos2d::Rect& area, int damage);

    // 检测触发：传入所有士兵，判断有没有人踩中
    bool checkTrigger(const cocos2d::Vector<Soldier*>& soldiers);

private:
    cocos2d::Rect _trapArea; // 陷阱生效的矩形区域
    int _damage;             // 伤害值
    bool _isExploded;        // 是否已经爆炸过
